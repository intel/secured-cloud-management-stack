...
EOF
    ...
    if [ ! -d ~/openstack/kolla-ansible/ansible/group_vars/control ]; then
        mkdir ~/openstack/kolla-ansible/ansible/group_vars/control
        cat << EOF > ~/openstack/kolla-ansible/ansible/group_vars/control/sgx.yml
---
enable_dcap: true
EOF
    fi
...
function create_vm() {
    ...
    # 4GB epc
    openstack quota set --sgx-epc 4096 ${project_id}
    ...
    if [ "$(openstack server list | grep sgx-virtual-instance)" = "" ]; then
        openstack server create --flavor sgx-2c-4g-1g --network provision-net \
    ...
    while true ; do
        ...
        if [ "$vm_ip" = "|" -o "$vm_ip" = "" ]; then
            echo "WARNING: Not get IP address of vm yet!"           
            sleep 10
        else
